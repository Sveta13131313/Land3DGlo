...
var sliderCarousel = function sliderCarousel() {
  ...
  var responseInit = function responseInit() {
    var slidesToShowDefault = slidesToShow,
        allRespone = responseInit.map(function (item) {
      return item.breakpoint;
    }),
        maxResponse = Math.max.apply(Math, _toConsumableArray(allRespone));
      ...
      if (widthWindow < maxResponse) {
        for (var i = 0; i < allRespone.length; i++) {
          if (widthWindow < allRespone[i]) {
            responsive[i].slidesToShow, _readOnlyError("slidesToShow");
            options.widthSlide = Math.floor(100 / slidesToShow);
            addStyle();
          } else {
            slidesToShowDefault, _readOnlyError("slidesToShow");
            options.widthSlide = Math.floor(100 / slidesToShow);
            addStyle();
          }
        }
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("301c8bd8c1926260b246")
/******/ })();
/******/ 
/******/ }
);